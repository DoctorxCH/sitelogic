...
export const renderChecklist = async (checklist, container) => {
    ...
    let allItems = await getFromDB('checklist_items');
    let items = allItems.filter(i => i.checklist_id === checklist.id);
    ...
    const calculateProgress = () => {
        ...
        // Mocking completion status on item for calculation (assuming item.completed exists later)
    ...
    const toggleItemSwitch = async (item, checked) => {
        item.kriterien_ausgeschaltet = !checked; // If checkbox is checked, it's active. If unchecked, it's N/A
    ...
    let itemsHtml = items.map(item => `
        <div class="border-t pt-2 mt-2 flex justify-between items-center" id="item-${item.id}">
            <div>
                <span class="font-medium">Item ${item.id}</span>
                <div class="media-container mt-2"></div>
            </div>
            <div>
                <label class="inline-flex items-center cursor-pointer">
                  <span class="mr-2 text-sm text-gray-600">Active (N/A Toggle)</span>
    ...
    wrapper.innerHTML = `
        <div class="flex justify-between items-center mb-4">
            <h4 class="font-bold text-lg">Checklist ${checklist.id}</h4>
            <div class="flex items-center space-x-4">
                ...
                <label class="inline-flex items-center cursor-pointer">
                  <span class="mr-2 font-semibold">Hauptschalter</span>
        ...
        <div class="w-full bg-gray-200 rounded-full h-4 mb-4 dark:bg-gray-700">
          <div class="bg-blue-600 h-4 rounded-full text-xs font-medium text-blue-100 text-center p-0.5 leading-none progress-bar" style="width: ${calculateProgress()}%"> ${calculateProgress()}%</div>
        </div>
        ...
        <div class="workflow-controls mt-4 pt-4 border-t border-gray-200"></div>
    `;
